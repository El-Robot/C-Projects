...
void checkValidity(char input[], int l)
{
  ...
  for (j = 0; j < l; j++)
  {
    ...
    if (currentValue == nextValue && (currentValue == 5 || currentValue == 50 || currentValue == 500))
      {
        printf("[Error:Roman numeral %s does not exist]", str);
        return;
      }
    ...
    /* if a numeral is larger than a previous numeral the whole numeral is not valid*/
